...
<task type="auto">
  <name>Task 3: Extend useAppState metadata and relax CommandCenter input gating</name>
  <files>frontend/src/hooks/useAppState.tsx, frontend/src/components/CommandCenter.tsx</files>
  <action>
...
4. **Modify handleSubmit to detect interactive commands:**
   ...
   ```typescript
   // Route to interactive flow when no data source is connected
   if (!hasDataSource) {
     try {
  ...
  <done>
    - ConversationMessage metadata supports formSchema for elicitation (backward compatible)
    - Command input enabled without data source for interactive commands
    - NL commands without data source route to interactive start endpoint
    - Existing batch flow completely unaffected (hasDataSource path unchanged)
  </done>
</task>

<task type="auto">
  <name>Task 4: Add Interactive Ship button, form handlers, and render DynamicFormRenderer in chat</name>
  <files>frontend/src/components/CommandCenter.tsx</files>
  <action>
This task adds the Interactive Ship button, form submission/cancel handlers, and renders the DynamicFormRenderer inline in the chat thread. It builds on the state and imports added in Task 3.

1. **Add Interactive Ship button in WelcomeMessage:**
...
2. **Add handleInteractiveShipClick handler:**
   ```typescript
...
3. **Add form submission and cancel handlers:**
   ```typescript
...
4. **Render DynamicFormRenderer in the messages area:**
...
5. **Update auto-scroll dependencies:**
...
- The `PackageIcon` SVG component already exists in CommandCenter.tsx -- reuse it.
  ...
  <done>
    ...
    - Button click starts interactive flow with full form schema
    - Form submission flows into standard PreviewCard -> confirm -> execute pipeline
    - Cancel dismisses form and restores chat input
    - DynamicFormRenderer renders inline in chat thread
    - Auto-scroll includes interactive form state
