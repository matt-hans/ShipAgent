# src/mcp/platforms/oracle/server.py
"""Oracle standalone platform MCP server.
...
from src.mcp.platforms.oracle.constants import (
    CONTRACT_VERSION,
    DEFAULT_PAGE_SIZE,
    MAX_CONCURRENCY,
    MAX_PAGE_SIZE,
...
_credentials: OracleCredentials | None = None

mcp = FastMCP("oracle-platform")
...
async def health() -> dict[str, Any]:
    """Check platform health: database reachability and auth validity."""
    result: dict[str, Any] = {
        "ok": False,
    ...
    try:
        ...
        if e.error_code == PlatformErrorCode.AUTH_EXPIRED:
            result["api_reachable"] = True  # DB reached, auth failed
...
async def auth_connect(
    ...
    """Connect to Oracle database with credentials.

    Args:
        credential_ref: Profile identifier (e.g., "primary").
        host: Oracle database hostname or IP.
        port: Oracle listener port (e.g., 1521).
        service_name: Oracle service name (e.g., "ORCL").
        user: Database username.
        password: Database password.
        orders_table: Table containing orders (default "SALES_ORDERS").
        tracking_table: Table for tracking write-back (default "SHIPMENT_TRACKING").
    """
    ...
    try:
        ...
        return {
            "connected": True,
            "auth_valid": True,
            "account_id": f"{host}:{port}/{service_name}",
            "account_label": f"{user}@{service_name}",
        }
    except PlatformError as e:
        return {
            "connected": False,
...
async def orders_list(
    ...
    """Fetch a page of orders with offset-based pagination.

    For Oracle, the cursor is the row offset as a string (e.g., "50", "100").
    First page uses offset 0 (cursor=None).

    Args:
        cursor: Offset string from previous response (for pagination).
    ...
    # Parse cursor as integer offset
    offset = 0
    if cursor is not None:
        try:
            ...
            return {
                ...
                "message": f"Invalid cursor (expected integer offset): {cursor}",
            }
...
async def orders_get(order_id: str) -> dict[str, Any]:
    ...
    Args:
        order_id: The Oracle order ID (ORDER_ID column value).
    """
    if _client is None:
        return _not_connected_error()
...
async def tracking_write_back(
    ...
    Inserts or updates a row in the tracking table with tracking details.

    Args:
        order_id: The Oracle order ID to update.
