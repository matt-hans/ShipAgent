...
def test_ensure_ups_specs_dir_accepts_pascalcase_optional_specs(tmp_path):
    """Optional specs are copied when source uses UPS upstream casing."""
    docs_dir = tmp_path / "docs"
    docs_dir.mkdir()
    (docs_dir / "rating.yaml").write_text(
        "openapi: 3.0.3\ninfo:\n  title: Rating\npaths: {}"
    ...
    (docs_dir / "LandedCost.yaml").write_text(
        "openapi: 3.0.3\ninfo:\n  title: LandedCost\npaths: {}"
    )
