...
lsof -ti:"${SHIPAGENT_PORT}" | xargs kill -9 2>/dev/null
...
.venv/bin/python -m uvicorn src.api.main:app --reload --port "${SHIPAGENT_PORT}" > backend.log 2>&1 &
...
# Reconnect Shopify
curl -s "http://localhost:${SHIPAGENT_PORT}/api/v1/platforms/shopify/env-status" > /dev/null

echo ""
echo "Stack restarted."
echo "  Backend:  http://localhost:${SHIPAGENT_PORT}"
